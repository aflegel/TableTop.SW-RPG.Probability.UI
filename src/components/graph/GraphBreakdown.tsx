...
type GraphBreakdownProps =
    IGraphBreakdownProps;        // ... state we've requested from the Redux store // ... plus incoming routing parameters
...
export default class GraphBreakdown extends React.Component<GraphBreakdownProps, {}> {
    ...
    public render = () => {
		...
		var positiveSet = this.props.baseSet.filter(f => f.quantity > 0);
		//success mode requires 0 quantity outcomes as well
		var negativeSet = this.props.baseSet.filter(f => (f.quantity < (this.props.mode == DieSymbol.Success ? 1 : 0)));
		...
		var average = this.props.baseSet.reduce((total, obj) => { return total + (obj.quantity * obj.frequency) }, 0) / this.props.totalFrequency;
		//val - avg squared * qty
		var deviationSet = this.props.baseSet.map(map => ((map.quantity - average) ** 2) * map.frequency);
		var standardDeviation = Math.sqrt(deviationSet.reduce((total, obj) => { return total + obj }, 0) / this.props.totalFrequency);

		var totalLabeling = this.props.mode == DieSymbol.Success ? "Total Frequency" : "";
		var totalData = this.props.mode == DieSymbol.Success ? new Intl.NumberFormat("en-Us").format(this.props.totalFrequency) : null;
